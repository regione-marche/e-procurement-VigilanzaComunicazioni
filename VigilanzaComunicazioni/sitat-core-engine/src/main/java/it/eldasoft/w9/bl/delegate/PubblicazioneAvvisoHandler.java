...
public class PubblicazioneAvvisoHandler extends AbstractRequestHandler {
  ...
  protected FaseEstesaType getFaseInvio(XmlObject documento) {
    RichiestaRichiestaRispostaSincronaPubblicazioneAvvisoDocument doc = (RichiestaRichiestaRispostaSincronaPubblicazioneAvvisoDocument) documento;
    return (FaseEstesaType) doc.getRichiestaRichiestaRispostaSincronaPubblicazioneAvviso().getFase();
  }
  ...
  protected void managePrimoInvio(XmlObject documento,
    ...
    if (!this.existsUffint(fase)) {
      logger.error("La richiesta di primo invio e' associata ad una stazione appaltante ("
          + fase.getW9FLCFSA()
          + ") non presente in DB");
      this.setEsito(
          datiAggiornamento,
          StatoComunicazione.STATO_ERRATA.getStato(),
          "Primo invio di pubblicazione avviso con stazione appaltante non presente in banca dati");
    ...
    if (this.existsAvviso(new Long(tab7.getW3PAVVISOID()), fase.getW9FLCFSA(), new Long(tab7.getW3PACODSIS()))) {
      logger.error("La richiesta di primo invio ha gia' un avviso con id:"
          + tab7.getW3PAVVISOID()
          + ", cfein:"
          + fase.getW9FLCFSA()
          + " e codsistema:"
      ...
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
          "Primo invio di pubblicazione avviso gia' presente in banca dati");
      // si termina con questo errore
      return;
    }
  ...
  protected void manageRettifica(XmlObject documento,
    ...
    if (!this.existsUffint(fase)) {
      logger.error("La richiesta di rettifica e' associata ad una stazione appaltante ("
          + fase.getW9FLCFSA()
          + ") non presente in DB");
      this.setEsito(
          datiAggiornamento,
          StatoComunicazione.STATO_ERRATA.getStato(),
          "Rettifica di pubblicazione avviso con stazione appaltante non presente in banca dati");
      // si termina con questo errore
      return;
    }
    
    if (!this.existsAvviso(new Long(tab7.getW3PAVVISOID()), fase.getW9FLCFSA(), new Long(tab7.getW3PACODSIS()))) {
      logger.error("La richiesta di rettifica non ha un avviso con id:"
          + tab7.getW3PAVVISOID()
          + ", cfein:"
          + fase.getW9FLCFSA()
          + " e codsistema:"
          ...
          + " presente in DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
          "Rettifica di pubblicazione avviso non presente in banca dati");
      // si termina con questo errore
      return;
    }
  ...
  private boolean existsLotto(String cig) throws SQLException {
    Long numeroOccorrenze = (Long) this.sqlManager.getObject(
        "select count(cig) from w9lott where cig = ?", new Object[] { cig });
  ...
  private boolean existsAvviso(Long idAvviso, String cfein, Long codsistema) throws SQLException {
    Long numeroOccorrenze = (Long) this.sqlManager.getObject(
        "select count(avviso.idavviso) from avviso, uffint "
         + "where idavviso=? and avviso.codein=uffint.codein and uffint.cfein=? " 
         + "and avviso.codsistema=?",
        new Object[] { idAvviso, cfein, codsistema });
    return numeroOccorrenze.intValue() > 0;
  }
  ...
  private void insertDatiFlusso(
      RichiestaRichiestaRispostaSincronaPubblicazioneAvvisoDocument doc,
    ...
    datiAggiornamento.addColumn("AVVISO.TIPOAVV",
        JdbcParametro.TIPO_NUMERICO, new Long(tab7.getW3PATAVVI().toString()));
  ...
  private void manageIntegrazioneORettifica(
    ...
    Long idAvviso = new Long(tab7.getW3PAVVISOID());
    Long codSistema = new Long(tab7.getW3PACODSIS());
    String cfein = fase.getW9FLCFSA();
    
    this.deleteDatiFlusso(idAvviso, cfein, codSistema);
   ...
   * @throws SQLException
   */
  private void deleteDatiFlusso(Long idAvviso, String cfein, Long codSistema) throws SQLException {
    String codein = (String) this.sqlManager.getObject(
        "select codein from uffint where cfein=?", new Object[]{cfein});

    this.sqlManager.update(
        "delete from avviso where idAvviso=? and codein=? and codsistema=?",
            new Object[] { idAvviso, codein, codSistema });
  }
