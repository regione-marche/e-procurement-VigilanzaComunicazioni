 ...
 * E' l'handler per la gestione della Gara per ente nazionale o importo
...
public class GaraEnteNazionaleSotto40000Handler extends AbstractRequestHandler {
	...
	protected boolean isTest(XmlObject documento) {
		RichiestaRichiestaRispostaSincronaGaraEnteNazionaleSotto40000Document doc = (RichiestaRichiestaRispostaSincronaGaraEnteNazionaleSotto40000Document) documento;
	...
	protected FaseType getFaseInvio(XmlObject documento) {
		RichiestaRichiestaRispostaSincronaGaraEnteNazionaleSotto40000Document doc = (RichiestaRichiestaRispostaSincronaGaraEnteNazionaleSotto40000Document) documento;
		return doc.getRichiestaRichiestaRispostaSincronaGaraEnteNazionaleSotto40000().getFase();
	}
	...
	protected void managePrimoInvio(XmlObject documento, DataColumnContainer datiAggiornamento, boolean ignoreWarnings) throws GestoreException, SQLException {
		...
		if (this.existsGara(tab31)) {
			logger.error("La richiesta di primo invio ha un id (" + tab31.getW9GNIDGARA() + ") di una gara per enti nazionali o sotto i 40.000 euro esistente in DB");
			this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(), "Primo invio " + "di una gara per enti nazionali o sotto i 40.000 euro gia' esistente in banca dati");
			return;
		}
	 ...
	 * @param tab31
	 *            Tab31Type
	 * @return Ritorna true, se esiste in W9GARA_ENTINAZ un record con idAvGara
	 *         pari a TAB31.W9GNIDGARA
	 * @throws SQLException
	 *             SQLException
	 */
	private boolean existsGara(final Tab31Type tab31) throws SQLException {
		...
		if (StringUtils.isNotEmpty(tab31.getW9GNIDGARA())) {
			...
			numeroOccorrenze = (Long) this.sqlManager.getObject("select count(CODGARA) from W9GARA_ENTINAZ where IDAVGARA = ?", new Object[] { idGara });
		}
	...
	/**
	 * Inserimento dei dati della gara, dei documenti del badno e dei lotti ed
	 * esiti.
	 * 
	 * @param doc
	 *            RichiestaRichiestaRispostaSincronaGaraEnteNazionaleSotto40000Document
	 * @param datiAggiornamento
	 *            DataColumnContainer
	 * @param primoInvio
	 *            Primo invio?
	 * @param codagaradeleted
	 *            Codice della gara cancellata (in casa di
	 ...
	 *             GestoreException
	 */
	private void insertDatiFlusso(final RichiestaRichiestaRispostaSincronaGaraEnteNazionaleSotto40000Document doc, final DataColumnContainer datiAggiornamento, final boolean primoInvio,
			final Long codagaradeleted) throws SQLException, GestoreException {

		Tab31Type tab31 = doc.getRichiestaRichiestaRispostaSincronaGaraEnteNazionaleSotto40000().getTab31();

		if (tab31.getArch1() != null) {
			String pkUffint = this.getStazioneAppaltante(tab31.getArch1());

			Long codgara = null;
			if (primoInvio) {
				codgara = new Long(this.genChiaviManager.getMaxId("W9GARA_ENTINAZ", "CODGARA") + 1);
			...
			if (tab31 != null) {
				...
				if (tab31.getW9GNTIPOAPP() != null) {
					datiAggiornamento.addColumn("W9GARA_ENTINAZ.TIPO_APP", JdbcParametro.TIPO_NUMERICO, Long.parseLong(tab31.getW9GNTIPOAPP().toString()));
				}
			...
			if (doc.getRichiestaRichiestaRispostaSincronaGaraEnteNazionaleSotto40000().sizeOfTab312Array() > 0) {
				...
				for (int iu = 0; iu < arrayTab312.length; iu++) {
					...
					if (tab312 != null) {
						...
						if (tab312.isSetFile()) {
							...
							try {
								...
								throw new GestoreException("Errore durante la lettura del file allegato al lotto-esito presente nella richiesta XML", null, e);
							}
					...
					// Rimozione dei campi di W9LOTT_ENTINAZ dal
					// DataColumnContainer per il successivo lotto
					datiAggiornamento.removeColumns(new String[] { "W9LOTT_ENTINAZ.CODGARA", "W9LOTT_ENTINAZ.CODLOTT", "W9LOTT_ENTINAZ.OGGETTO", "W9LOTT_ENTINAZ.IMPORTO_TOT", "W9LOTT_ENTINAZ.CIG",
							"W9LOTT_ENTINAZ.CPV", "W9LOTT_ENTINAZ.ID_CATEGORIA_PREVALENTE", "W9LOTT_ENTINAZ.CLASCAT", "W9LOTT_ENTINAZ.IMP_AGG", "W9LOTT_ENTINAZ.IMPORTO_AGGIUDICAZIONE",
