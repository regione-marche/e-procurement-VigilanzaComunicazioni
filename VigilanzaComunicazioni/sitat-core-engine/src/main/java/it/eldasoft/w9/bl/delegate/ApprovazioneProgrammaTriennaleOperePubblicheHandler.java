...
import it.toscana.rete.rfc.sitat.types.OperaIncompiutaType;
import it.toscana.rete.rfc.sitat.types.ProgrammaOperePubblicheType;
import it.toscana.rete.rfc.sitat.types.RichiestaRichiestaRispostaSincronaApprovazioneProgrammaTriennaleOperePubblicheDocument;
 ...
 */
public class ApprovazioneProgrammaTriennaleOperePubblicheHandler extends AbstractRequestHandler {

  //TODO Classe da rimuovere
  
  @Override
  protected String getNomeFlusso() {
    return "Programma triennale opere pubbliche";
  }

  @Override
  protected String getMainTagRequest() {
    return CostantiFlussi.MAIN_TAG_RICHIESTA_APPROVAZIONE_PROGRAMMA_TRIENNALE_OPERE_PUBBLICHE;
  }

  @Override
  protected XmlObject getXMLDocument(String xmlEvento) throws XmlException {
    return RichiestaRichiestaRispostaSincronaApprovazioneProgrammaTriennaleOperePubblicheDocument.Factory.parse(xmlEvento);
  }

  @Override
  protected boolean isTest(XmlObject documento) {
    RichiestaRichiestaRispostaSincronaApprovazioneProgrammaTriennaleOperePubblicheDocument doc = (RichiestaRichiestaRispostaSincronaApprovazioneProgrammaTriennaleOperePubblicheDocument) documento;
    return doc.getRichiestaRichiestaRispostaSincronaApprovazioneProgrammaTriennaleOperePubbliche().getTest();
  }

  @Override
  protected FaseEstesaType getFaseInvio(XmlObject documento) {
    RichiestaRichiestaRispostaSincronaApprovazioneProgrammaTriennaleOperePubblicheDocument doc = (RichiestaRichiestaRispostaSincronaApprovazioneProgrammaTriennaleOperePubblicheDocument) documento;
    return (FaseEstesaType) doc.getRichiestaRichiestaRispostaSincronaApprovazioneProgrammaTriennaleOperePubbliche().getFase();
  }
  ...
  protected void managePrimoInvio(XmlObject documento, DataColumnContainer datiAggiornamento, boolean ignoreWarnings)
    ...
    RichiestaRichiestaRispostaSincronaApprovazioneProgrammaTriennaleOperePubblicheDocument doc = (RichiestaRichiestaRispostaSincronaApprovazioneProgrammaTriennaleOperePubblicheDocument) documento;
    ProgrammaOperePubblicheType programma = doc.getRichiestaRichiestaRispostaSincronaApprovazioneProgrammaTriennaleOperePubbliche().getProgramma();
  ...
  protected void manageRettifica(XmlObject documento, DataColumnContainer datiAggiornamento, boolean ignoreWarnings)
    ...
    if (!this.isProgrammaLavori(programma.getT2IDCONTRI())) {
    ...
    this.deleteDatiFlusso(
        doc.getRichiestaRichiestaRispostaSincronaApprovazioneProgrammaTriennaleOperePubbliche().getProgramma().getT2IDCONTRI());
    this.insertDatiFlusso(doc, datiAggiornamento);
  }
   ...
   * del programma ricevuto nel flusso, ovvero lavori.
   * 
   * @param idContri
   * @return true se il programma nel DB e' di lavori, false altrimenti (forniture e servizi)
   * @throws SQLException SQLException
   */
  private boolean isProgrammaLavori(final String idContri) throws SQLException {
    Long tiprog = (Long) this.sqlManager.getObject(
        "select tiprog from piatri where id = ?", new Object[] { idContri });
    return tiprog.intValue() == 1;
  }
  ...
  protected void insertDatiFlusso(final 
      RichiestaRichiestaRispostaSincronaApprovazioneProgrammaTriennaleOperePubblicheDocument doc,
    ...
    datiAggiornamento.addColumn("PIATRI.TIPROG", JdbcParametro.TIPO_NUMERICO, tiprog);
    ...
    // ciclo sulle opere incompiute (OITRI)
    if (programma.getOpereIncompiuteArray() != null && programma.getOpereIncompiuteArray().length > 0) { 
      for (int i=0; i < programma.getOpereIncompiuteArray().length; i++) {
         OperaIncompiutaType operaIncompiuta = programma.getOpereIncompiuteArray(i);
         if (operaIncompiuta != null) {
           ...
           DataColumnContainer dccOperaIncompiuta = new DataColumnContainer(
               new DataColumn[] {
                   new DataColumn("OITRI.CONTRI", new JdbcParametro(JdbcParametro.TIPO_NUMERICO, idPiatri)),
                   new DataColumn("OITRI.NUMOI", new JdbcParametro(JdbcParametro.TIPO_NUMERICO, 
                       operaIncompiuta.getNUMOIOITRIP())) });
           
           if (operaIncompiuta.isSetAMBIINTOITR()) {
             dccOperaIncompiuta.addColumn("OITRI.AMBITOINT", JdbcParametro.TIPO_TESTO, 
                 operaIncompiuta.getAMBIINTOITR().toString());
           }
           if (operaIncompiuta.isSetANNULTQEOIT()) {
             dccOperaIncompiuta.addColumn("OITRI.ANNOULTQE", JdbcParametro.TIPO_NUMERICO, new Long(operaIncompiuta.getANNULTQEOIT()));
           }
           if (operaIncompiuta.isSetAVANZOITRIP()) {
             dccOperaIncompiuta.addColumn("OITRI.AVANZAMENTO", JdbcParametro.TIPO_DECIMALE, 
                 new Double(new Float(operaIncompiuta.getAVANZOITRIP())));
           }
           if (operaIncompiuta.isSetCAUSAOITRIP()) {
             dccOperaIncompiuta.addColumn("OITRI.CAUSA", JdbcParametro.TIPO_TESTO,
                 operaIncompiuta.getCAUSAOITRIP().toString());
           }
           if (operaIncompiuta.isSetCESSOITRIPI()) {
             dccOperaIncompiuta.addColumn("OITRI.CESSIONE", JdbcParametro.TIPO_NUMERICO,
                 this.getFlagSNDB(operaIncompiuta.getCESSOITRIPI()).toString());
           }
           if (operaIncompiuta.isSetCOSTOOITRP()) {
             dccOperaIncompiuta.addColumn("OITRI.COSTO", JdbcParametro.TIPO_DECIMALE,
                 operaIncompiuta.getCOSTOOITRP());
           }
           if (operaIncompiuta.isSetCPAPOITRI()) {
             dccOperaIncompiuta.addColumn("OITRI.COP_ALTRAPUBBLICA", JdbcParametro.TIPO_TESTO, 
                 this.getFlagSNDB(operaIncompiuta.getCPAPOITRI()));
           }
           if (operaIncompiuta.isSetCPCMOITRI()) {
             dccOperaIncompiuta.addColumn("OITRI.COP_COMUNITARIA", JdbcParametro.TIPO_TESTO, 
                 this.getFlagSNDB(operaIncompiuta.getCPCMOITRI()));
           }
           if (operaIncompiuta.isSetCPCOOITRI()) {
             dccOperaIncompiuta.addColumn("OITRI.COP_COMUNALE", JdbcParametro.TIPO_TESTO, 
                 this.getFlagSNDB(operaIncompiuta.getCPCOOITRI()));
           }
           if (operaIncompiuta.isSetCPPROITRI()) {
             dccOperaIncompiuta.addColumn("OITRI.COP_PROVINCIALE", JdbcParametro.TIPO_TESTO, 
                 this.getFlagSNDB(operaIncompiuta.getCPPROITRI()));
           }
           if (operaIncompiuta.isSetCPPVOITRI()) {
             dccOperaIncompiuta.addColumn("OITRI.COP_PRIVATA", JdbcParametro.TIPO_TESTO, 
                 this.getFlagSNDB(operaIncompiuta.getCPPVOITRI()));
           }
           if (operaIncompiuta.isSetCPREOITRI()) {
             dccOperaIncompiuta.addColumn("OITRI.COP_REGIONALE", JdbcParametro.TIPO_TESTO, 
                 this.getFlagSNDB(operaIncompiuta.getCPREOITRI()));
           }
           if (operaIncompiuta.isSetCPSTOITRI()) {
             dccOperaIncompiuta.addColumn("OITRI.COP_STATALE", JdbcParametro.TIPO_TESTO, 
                 this.getFlagSNDB(operaIncompiuta.getCPSTOITRI()));
           }
           if (operaIncompiuta.isSetCUPMASTEROI()) {
             dccOperaIncompiuta.addColumn("OITRI.CUPMASTER", JdbcParametro.TIPO_TESTO, 
                 operaIncompiuta.getCUPMASTEROI().toString());
           }
           if (operaIncompiuta.isSetCUPOITRIPIA()) {
             dccOperaIncompiuta.addColumn("OITRI.CUP", JdbcParametro.TIPO_TESTO,
                 operaIncompiuta.getCUPOITRIPIA());
           }
           if (operaIncompiuta.isSetDEMOLOITRIP()) {
             dccOperaIncompiuta.addColumn("OITRI.DEMOLIZIONE", JdbcParametro.TIPO_TESTO, 
                 this.getFlagSNDB(operaIncompiuta.getDEMOLOITRIP()));
           }
           if (operaIncompiuta.isSetDESCROITRIP()) {
             dccOperaIncompiuta.addColumn("OITRI.DESCRIZIONE", JdbcParametro.TIPO_TESTO,
                 operaIncompiuta.getDESCROITRIP());
           }
           if (operaIncompiuta.isSetDESTUSOOITR()) {
             dccOperaIncompiuta.addColumn("OITRI.DESTINAZIONEUSO", JdbcParametro.TIPO_TESTO,
                 operaIncompiuta.getDESTUSOOITR().toString());
           }
           if (operaIncompiuta.isSetDETERMOITRI()) {
             dccOperaIncompiuta.addColumn("OITRI.DETERMINAZIONI", JdbcParametro.TIPO_TESTO,
                 operaIncompiuta.getDETERMOITRI().toString());
           }
           if (operaIncompiuta.isSetDIMQTOITRP()) {
             dccOperaIncompiuta.addColumn("OITRI.DIM_QTA", JdbcParametro.TIPO_DECIMALE,
                 operaIncompiuta.getDIMQTOITRP());
           }
           if (operaIncompiuta.isSetDIMUMOITRIP()) {
             dccOperaIncompiuta.addColumn("OITRI.DIM_UM", JdbcParametro.TIPO_TESTO,
                 operaIncompiuta.getDIMUMOITRIP());
           }
           if (operaIncompiuta.isSetFINANOITRP()) {
             dccOperaIncompiuta.addColumn("OITRI.FINANZIAMENTO", JdbcParametro.TIPO_DECIMALE,
                 operaIncompiuta.getFINANOITRP());
           }
           if (operaIncompiuta.isSetFINPRGOITRI()) {
             dccOperaIncompiuta.addColumn("OITRI.FINANZA_PROGETTO", JdbcParametro.TIPO_TESTO, 
                 this.getFlagSNDB(operaIncompiuta.getFINPRGOITRI()));
           }
           if (operaIncompiuta.isSetFRUIBOITRIP()) {
             dccOperaIncompiuta.addColumn("OITRI.FRUIBILE", JdbcParametro.TIPO_TESTO, 
                 this.getFlagSNDB(operaIncompiuta.getFRUIBOITRIP()));
           }
           if (operaIncompiuta.isSetIMPINTOITRP()) {
             dccOperaIncompiuta.addColumn("OITRI.IMPORTOINT", JdbcParametro.TIPO_DECIMALE,
                 operaIncompiuta.getIMPINTOITRP());
           }
           if (operaIncompiuta.isSetIMPLAVOITRP()) {
             dccOperaIncompiuta.addColumn("OITRI.IMPORTOLAV", JdbcParametro.TIPO_DECIMALE,
                 operaIncompiuta.getIMPLAVOITRP());
           }
           if (operaIncompiuta.isSetIMPSALOITRP()) {
             dccOperaIncompiuta.addColumn("OITRI.IMPORTOSAL", JdbcParametro.TIPO_DECIMALE,
                 operaIncompiuta.getIMPSALOITRP());
           }
           if (operaIncompiuta.isSetINFRASTROIT()) {
             dccOperaIncompiuta.addColumn("OITRI.INFRASTRUTTURA", JdbcParametro.TIPO_TESTO, 
                 this.getFlagSNDB(operaIncompiuta.getINFRASTROIT()));
           }
           if (operaIncompiuta.isSetISTATOIT()) {
             dccOperaIncompiuta.addColumn("OITRI.ISTAT", JdbcParametro.TIPO_TESTO,
                 operaIncompiuta.getISTATOIT());
           }
           if (operaIncompiuta.isSetNUTCODOI()) {
             dccOperaIncompiuta.addColumn("OITRI.NUTS", JdbcParametro.TIPO_TESTO,
                 operaIncompiuta.getNUTCODOI());
           }
           if (operaIncompiuta.isSetONERULTOITR()) {
             dccOperaIncompiuta.addColumn("OITRI.ONERIULTIM", JdbcParametro.TIPO_DECIMALE,
                 operaIncompiuta.getONERULTOITR());
           }
           if (operaIncompiuta.isSetONESIOITRP()) {
             dccOperaIncompiuta.addColumn("OITRI.ONERI_SITO", JdbcParametro.TIPO_DECIMALE,
                 operaIncompiuta.getONESIOITRP());
           }
           if (operaIncompiuta.isSetREQCAPOITRI()) {
             dccOperaIncompiuta.addColumn("OITRI.REQ_CAP", JdbcParametro.TIPO_TESTO, 
                 this.getFlagSNDB(operaIncompiuta.getREQCAPOITRI()));
           }
           if (operaIncompiuta.isSetREQPRGOITRI()) {
             dccOperaIncompiuta.addColumn("OITRI.REQ_PRGE", JdbcParametro.TIPO_TESTO, 
                 this.getFlagSNDB(operaIncompiuta.getREQPRGOITRI()));
           }
           if (operaIncompiuta.isSetSPONSOITRIP()) {
             dccOperaIncompiuta.addColumn("OITRI.SPONSORIZZAZIONE", JdbcParametro.TIPO_TESTO, 
                 this.getFlagSNDB(operaIncompiuta.getSPONSOITRIP()));
           }
           if (operaIncompiuta.isSetSTATREALOIT()) {
             dccOperaIncompiuta.addColumn("OITRI.STATOREAL", JdbcParametro.TIPO_TESTO, 
                 operaIncompiuta.getSTATREALOIT().toString());
           }
           if (operaIncompiuta.isSetT2INTEOI()) {
             dccOperaIncompiuta.addColumn("OITRI.INTERV", JdbcParametro.TIPO_TESTO, 
                 operaIncompiuta.getT2INTEOI().toString());
           }
           if (operaIncompiuta.isSetT2SEZOIT()) {
             dccOperaIncompiuta.addColumn("OITRI.SEZINT", JdbcParametro.TIPO_TESTO, 
                 operaIncompiuta.getT2SEZOIT().toString());
           }
           if (operaIncompiuta.isSetUTILRIDOITR()) {
             dccOperaIncompiuta.addColumn("OITRI.UTILIZZORID", JdbcParametro.TIPO_NUMERICO, 
                 this.getFlagSNDB(operaIncompiuta.getUTILRIDOITR()));
           }
           if (operaIncompiuta.isSetVENDOITRIPI()) {
             dccOperaIncompiuta.addColumn("OITRI.VENDITA", JdbcParametro.TIPO_NUMERICO, 
                 this.getFlagSNDB(operaIncompiuta.getVENDOITRIPI()));
           }
           
           // Inserimento dell'opera incompiua (OITRI)
           dccOperaIncompiuta.insert("OITRI", sqlManager);
        }
      }
    }
        ...
        if (intervento.getImmobiliArray() != null && intervento.getImmobiliArray().length > 0) {
          for (int j=0; j < intervento.getImmobiliArray().length; j++) {
            ...
            if (immobile != null) {
              ...
              if (immobile.isSetNUMOIIMMTRAI()) {
                dccImmobile.addColumn("IMMTRAI.NUMOI", JdbcParametro.TIPO_NUMERICO, immobile.getNUMOIIMMTRAI());
              }
          ...
          for (int j=0; j < intervento.getRisorseCapitoloBilancioArray().length; j++) {
            ...
            if (risorsaCapitoloBilancio != null) {
              ...
              if (risorsaCapitoloBilancio.isSetT2VERBILCB()) {
                dccRisorsaCapitolo.addColumn("RIS_CAPITOLO.VERIFBIL", JdbcParametro.TIPO_DECIMALE,
                  risorsaCapitoloBilancio.getT2VERBILCB());
              }
