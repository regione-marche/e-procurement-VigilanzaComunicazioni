...
public class GestoreW9LOTTENTINAZ extends AbstractGestoreChiaveNumerica {
      ...
      try {
        for (int i = 1; i <= numeroRecord; i++) {
          ...
          newDataColumnContainer.removeColumns(new String[] {
              ...
              gestore.getEntita() + "." + "FILEDAALLEGARE"});
            ...
            if (updateOccorrenza) {
              ...
              if (formFile != null && StringUtils.isNotEmpty(formFile.getFileName())) {
                if (formFile.getFileSize() > 0 && formFile.getFileSize() <= MAX_FILE_SIZE) {
                  ...
                  newDataColumnContainer.addColumn("W9LOTT_ENTINAZ.FILE_ALLEGATO",
                ...
                // Se il formFile e' null, allora potrebbe essere che all'occorrenza del lotto ed esito sia
                // stata privata del file di esito allegato. In questo cosa bisogna sbiancare il campo BLOB su DB.
                // Il campo W9LOTT_ENTINAZ.FILEDAALLEGARE e' valorizzato 'canc'.
                if (tmpDataColumnContainer.getColumn(gestore.getEntita() + "." + "FILEDAALLEGARE_" + i) != null) { 
                  String tmpStr = tmpDataColumnContainer.getColumn(gestore.getEntita()
                      + "." + "FILEDAALLEGARE_" + i).getValue().getStringValue();
                  if ("CANC".equalsIgnoreCase(tmpStr)) {
                    
                    ByteArrayOutputStream baos = new ByteArrayOutputStream();
                    baos.write(new byte[]{'a', 'b', 'c'});
      
                    newDataColumnContainer.addColumn("W9LOTT_ENTINAZ.FILE_ALLEGATO",
                        new JdbcParametro(JdbcParametro.TIPO_BINARIO, null));
                    newDataColumnContainer.getColumn("W9LOTT_ENTINAZ.FILE_ALLEGATO").setObjectOriginalValue(baos);
                  }
                }
              }
