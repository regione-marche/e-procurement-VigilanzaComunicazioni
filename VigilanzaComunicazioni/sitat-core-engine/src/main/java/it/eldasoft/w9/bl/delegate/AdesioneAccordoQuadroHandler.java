...
public class AdesioneAccordoQuadroHandler extends AbstractRequestHandler {
  ...
  protected FaseEstesaType getFaseInvio(XmlObject documento) {
    RichiestaRichiestaRispostaSincronaAdesioneAccordoQuadroDocument doc =
      (RichiestaRichiestaRispostaSincronaAdesioneAccordoQuadroDocument) documento;
  ...
  protected void managePrimoInvio(XmlObject documento,
    ...
    RichiestaRichiestaRispostaSincronaAdesioneAccordoQuadroDocumentImpl doc =
      (RichiestaRichiestaRispostaSincronaAdesioneAccordoQuadroDocumentImpl) documento;
    ...
    if (!this.existsLotto(fase)) {
      ...
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      		"Primo invio di adesione accordo quadro/accordo quadro/convenzione di un lotto non presente in banca dati");
      continua = false;
    }

    if (continua && this.existsAppalto(fase)) {
      logger.error("La richiesta di primo invio di un lotto gia' aggiudicato sul DB per l'aggiudicazione "
      		+ fase.getW9NUMAPPA() + "(cig = " + fase.getW3FACIG() + ")");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
        "Primo invio di adesione accordo quadro di un lotto/accordo quadro/convenzione gia' aggiudicato in banca dati");
      continua = false;
    }

    // warnings
    if (continua && !ignoreWarnings) {
    	if (! this.isFaseVisualizzabile(fase, CostantiW9.ADESIONE_ACCORDO_QUADRO)) {
        logger.error("Primo invio adesione accordo quadro/accordo quadro/convenzione non prevista (AAQ and EA)");
        this.setEsito(datiAggiornamento, StatoComunicazione.STATO_WARNING.getStato(),
            "Primo invio adesione accordo quadro/accordo quadro/convenzione non prevista");
  ...
  private void insertDatiFlusso(
      RichiestaRichiestaRispostaSincronaAdesioneAccordoQuadroDocument doc,
      DataColumnContainer datiAggiornamento, boolean primoInvio, final String codcui)
  throws SQLException, GestoreException {
    ...
    if (primoInvio) {
	    ...
	    if (tab29.isSetW9FASCHLOC()) {
	    	idSchedaLocale = tab29.getW9FASCHLOC();
	  	}
	  	if (tab29.isSetW9FASCHSIM()) {
	  		idSchedaSimog = tab29.getW9FASCHSIM();
	  	}
    ...
    DataColumnContainer dccAppalto = new DataColumnContainer(new DataColumn[] {
        new DataColumn("W9APPA.CODGARA", new JdbcParametro(JdbcParametro.TIPO_NUMERICO, codGara)),
        new DataColumn("W9APPA.CODLOTT", new JdbcParametro(JdbcParametro.TIPO_NUMERICO, codLott)),
        new DataColumn("W9APPA.NUM_APPA", new JdbcParametro(JdbcParametro.TIPO_NUMERICO,
    ...
    if (StringUtils.isNotEmpty(codcui)) {
  	...
  	} else if (primoInvio && tab29.isSetW9APPACODCUI()) {
   		dccAppalto.addColumn("W9APPA.CODCUI", JdbcParametro.TIPO_TESTO, tab29.getW9APPACODCUI());
    }
    ...
    if (tab29.isSetW3PERCRIB()) {
      dccAppalto.addColumn("W9APPA.PERC_RIBASSO_AGG", JdbcParametro.TIPO_DECIMALE, new Double(
          new Float(tab29.getW3PERCRIB()).toString()));
    }
    if (tab29.isSetW3PERCOFF()) {
      dccAppalto.addColumn("W9APPA.PERC_OFF_AUMENTO", JdbcParametro.TIPO_DECIMALE,
          new Double(new Float(tab29.getW3PERCOFF()).toString()));
    }
    ...
    dccAppalto.addColumn("W9APPA.FLAG_RICH_SUBAPPALTO", JdbcParametro.TIPO_TESTO,
        this.getFlagSNDB(tab29.getW3FLAGRIC()).toString());
    
    if (tab29.isSetW9APTIPAT()) {
      dccAppalto.addColumn("W9APPA.TIPO_ATTO", JdbcParametro.TIPO_NUMERICO, 
        Long.parseLong(tab29.getW9APTIPAT().toString()));
    }
    ...
    if (tab29.isSetW3INONAS()) {
      dccAppalto.addColumn("W9APPA.IMP_NON_ASSOG", JdbcParametro.TIPO_DECIMALE, tab29.getW3INONAS());
    }
    ...
    if (StringUtils.isEmpty(dbCodCuiLotto) && primoInvio && tab29.isSetW9APPACODCUI()) {
    	this.sqlManager.update("update W9LOTT set CODCUI=? where CODGARA=? and CODLOTT=?", 
    			new Object[] { tab29.getW9APPACODCUI(), codGara, codLott } );
    }
    ...
    for (int i = 0; i < doc.getRichiestaRichiestaRispostaSincronaAdesioneAccordoQuadro().getListaTab291().sizeOfTab291Array(); i++) {
      ...
      // popolamento dei dati per l'inserimento di un intervento (tab291): si usa
      ...
      DataColumnContainer dccAggiudicazione = new DataColumnContainer(new DataColumn[] {
          new DataColumn("W9AGGI.CODGARA", new JdbcParametro(JdbcParametro.TIPO_NUMERICO, codGara)),
      ...
      dccAggiudicazione.addColumn("W9AGGI.ID_TIPOAGG", JdbcParametro.TIPO_NUMERICO,
          new Long(tab291.getW3IDTIPOA().toString()));
      if (tab291.isSetW3RUOLO()) {
        dccAggiudicazione.addColumn("W9AGGI.RUOLO", JdbcParametro.TIPO_NUMERICO,
            new Long(tab291.getW3RUOLO().toString()));
      }
      dccAggiudicazione.addColumn("W9AGGI.FLAG_AVVALIMENTO", JdbcParametro.TIPO_NUMERICO,
          new Long(tab291.getW3FLAGAVV().toString()));
      ...
      if (tab291.isSetW3AGIDGRP()) {
    	  dccAggiudicazione.addColumn("W9AGGI.ID_GRUPPO",
    		JdbcParametro.TIPO_NUMERICO, new Long(tab291.getW3AGIDGRP()));
      }
      
      if (tab291.isSetW3AGIMPAGGI()) {
    	  dccAggiudicazione.addColumn("W9AGGI.IMPORTO_AGGIUDICAZIONE",
    		JdbcParametro.TIPO_DECIMALE, new Double(tab291.getW3AGIMPAGGI()));
      }
      
      if (tab291.isSetW3AGPERCRIB()) {
    	  dccAggiudicazione.addColumn("W9AGGI.PERC_RIBASSO_AGG",
    		JdbcParametro.TIPO_DECIMALE, new Double(tab291.getW3AGPERCRIB()));
      }
      
      if (tab291.isSetW3AGPERCOFF()) {
    	  dccAggiudicazione.addColumn("W9AGGI.PERC_OFF_AUMENTO",
    		JdbcParametro.TIPO_DECIMALE, new Double(tab291.getW3AGPERCOFF()));
      }
    ...
    // ciclo sulgli incarichi professionali (W9INCA)
    ...
    for (int i = 0; i < doc.getRichiestaRichiestaRispostaSincronaAdesioneAccordoQuadro().getListaTab292().sizeOfTab292Array(); i++) {
      ...
      // popolamento dei dati per l'inserimento di un intervento (tab292): si usa
      // un altro container dato che non e' un singolo elemento ma una lista di
      // elementi da inserire nella medesima tabella
      ...
      if (tab292.getW3SEZIONE().equals(sezione)) {
        k = k + 1;
        DataColumnContainer dccIncarichi = new DataColumnContainer(new DataColumn[] {
            ...
            new DataColumn("W9INCA.NUM", new JdbcParametro(JdbcParametro.TIPO_NUMERICO,
                new Long(fase.getW3NUM5()))),
        ...
        dccIncarichi.addColumn("W9INCA.ID_RUOLO", JdbcParametro.TIPO_NUMERICO,
            new Long(tab292.getW3IDRUOLO().toString()));
        if (tab292.isSetW3CIGPROG()) {
          dccIncarichi.addColumn("W9INCA.CIG_PROG_ESTERNA", JdbcParametro.TIPO_TESTO,
              tab292.getW3CIGPROG());
        }
        if (tab292.isSetW3DATAAFF()) {
          dccIncarichi.addColumn("W9INCA.DATA_AFF_PROG_ESTERNA", JdbcParametro.TIPO_DATA,
              tab292.getW3DATAAFF().getTime());
        }
        if (tab292.isSetW3DATACON()) {
          dccIncarichi.addColumn("W9INCA.DATA_CONS_PROG_ESTERNA", JdbcParametro.TIPO_DATA,
              tab292.getW3DATACON().getTime());
        }
        // si richiama l'inserimento dell'occorrenza di tab292 (W9INCA)
        dccIncarichi.insert("W9INCA", this.sqlManager);
      }
    }

    // ciclo sui finanziamenti (W9FINA)
    ...
    for (int i = 0; i < doc.getRichiestaRichiestaRispostaSincronaAdesioneAccordoQuadro().getListaTab293().sizeOfTab293Array(); i++) {
      ...
      // popolamento dei dati per l'inserimento di un intervento (tab293): si  usa
      // un altro container dato che non e' un singolo elemento ma una lista di
      // elementi da inserire nella medesima tabella

      // si costruisce il container inserendo i campi chiave dell'entita' da salvare
      DataColumnContainer dccFinanziamenti = new DataColumnContainer(new DataColumn[] {
          new DataColumn("W9FINA.CODGARA", new JdbcParametro(JdbcParametro.TIPO_NUMERICO, codGara)),
      ...
      dccFinanziamenti.addColumn("W9FINA.ID_FINANZIAMENTO",
          JdbcParametro.TIPO_TESTO, tab293.getW3IDFINAN().toString());
      
      
      dccFinanziamenti.addColumn("W9FINA.IMPORTO_FINANZIAMENTO",
          JdbcParametro.TIPO_DECIMALE, new Double(new Float(tab293.getW3IFINANZ()).toString()));
  ...
  private void manageRettificaOIntegrazione(XmlObject documento, DataColumnContainer datiAggiornamento,
    ...
    if (!this.existsLotto(fase)) {
      logger.error("La richiesta di " + this.getDescrioneTipoInvio(tipoInvio) +
          " ha un cig (" + fase.getW3FACIG() + ") di un lotto non presente in DB");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      		"Rettifica adesione accordo quadro/accordo quadro/convenzione di un lotto non presente in banca dati");
      continua = false;
    }

    if (continua && !this.existsFase(fase, CostantiW9.ADESIONE_ACCORDO_QUADRO)) {
      logger.error("La richiesta di rettifica di adesione accordo quadro di un lotto non aggiudicato in DB (cig = "
      		+ fase.getW3FACIG() + ")");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
     			"Rettifica di adesione accordo quadro/accordo quadro/convenzione non esistente nella banca dati");
      continua = false;
    }

    if (continua) {
    	String codcui = this.deleteDatiFlusso(doc.getRichiestaRichiestaRispostaSincronaAdesioneAccordoQuadro().getFase());
  ...
  private String deleteDatiFlusso(FaseEstesaType fase) throws SQLException, GestoreException {
    ...
    this.sqlManager.update(
        "delete from w9appa where codgara=? and codlott=? and num_appa = ?", new Object[]{codGara, codLott, new Long(fase.getW9NUMAPPA()) }, 1);
    this.sqlManager.update(
        "delete from w9aggi where codgara=? and codlott=? and num_appa = ?", new Object[]{codGara, codLott, new Long(fase.getW9NUMAPPA()) });
    this.sqlManager.update(
        "delete from w9inca where codgara=? and codlott=? and num = ? and sezione=?",
        new Object[] {codGara, codLott, new Long(fase.getW3NUM5()), "RQ"});
    this.sqlManager.update(
        "delete from w9fina where codgara=? and codlott=? and num_appa = ?", new Object[]{codGara, codLott, new Long(fase.getW9NUMAPPA())});
    return codcui;
  }
