...
import it.toscana.rete.rfc.sitat.types.Tab1L1Type;
...
public class ApprovazioneProgrammaLavoriHandler extends AbstractRequestHandler {
  ...
  protected FaseEstesaType getFaseInvio(XmlObject documento) {
    RichiestaRichiestaRispostaSincronaApprovazioneProgrammaLavoriDocument doc = (RichiestaRichiestaRispostaSincronaApprovazioneProgrammaLavoriDocument) documento;
    return (FaseEstesaType) doc.getRichiestaRichiestaRispostaSincronaApprovazioneProgrammaLavori().getFase();
  }
  ...
  protected void managePrimoInvio(XmlObject documento,
      ...
      SQLException {
    RichiestaRichiestaRispostaSincronaApprovazioneProgrammaLavoriDocument doc = (RichiestaRichiestaRispostaSincronaApprovazioneProgrammaLavoriDocument) documento;
    Tab1L1Type tab1 = doc.getRichiestaRichiestaRispostaSincronaApprovazioneProgrammaLavori().getTab1();
    ...
    /*for (int i = 0; i < doc.getRichiestaRichiestaRispostaSincronaApprovazioneProgrammaLavori().getListaTab2().sizeOfTab2Array(); i++) {
      Tab2LType tab2 = doc.getRichiestaRichiestaRispostaSincronaApprovazioneProgrammaLavori().getListaTab2().getTab2Array(i);

      if (tab2.isSetT2CATINT() && tab2.isSetT2INTERV()) {
        if (!this.existsCategoriaSottocategoriaIntervento(tab2)) {
          logger.error("L'intervento "
              + (i + 1)
              + " della richiesta ha il codice categoria ("
              + tab2.getT2INTERV().toString()
              + ") e sottocategoria ("
              + tab2.getT2CATINT()
   ...
   * @throws SQLException
   */
  private boolean existsProgramma(final Tab1L1Type tab1) throws SQLException {
   ...
   */
  /*private boolean existsCategoriaSottocategoriaIntervento(final Tab2LType tab2)
      throws SQLException {
    Long numeroOccorrenze = (Long) this.sqlManager.getObject(
  ...
  private void insertDatiFlusso(final 
      RichiestaRichiestaRispostaSincronaApprovazioneProgrammaLavoriDocument doc,
    ...
    if (tab1.isSetT2NOTSCH2()) {
      datiAggiornamento.addColumn("PIATRI.NOTSCHE2", JdbcParametro.TIPO_TESTO, tab1.getT2NOTSCH2());
    }
    if (tab1.isSetT2NOTSCH2B()) {
      datiAggiornamento.addColumn("PIATRI.NOTSCHE2B", JdbcParametro.TIPO_TESTO, tab1.getT2NOTSCH2B());
    }
    if (tab1.isSetT2NOTSCH3()) {
      datiAggiornamento.addColumn("PIATRI.NOTSCHE3", JdbcParametro.TIPO_TESTO, tab1.getT2NOTSCH3());
    }
    ...
    if (listaTab2Bis1 != null && listaTab2Bis1.sizeOfTab2Array() > 0) {
      for (int j = 0; j < listaTab2Bis1.sizeOfTab2Array(); j++) {
        ...
        // si costruisce il container inserendo i campi chiave dell'entita' da
        // salvare
        DataColumnContainer dccEcoTri = new DataColumnContainer(
            new DataColumn[] {
                new DataColumn("ECOTRI.CONTRI", new JdbcParametro(
                    JdbcParametro.TIPO_NUMERICO, idPiatri)),
                new DataColumn("ECOTRI.CONECO", new JdbcParametro(
                    JdbcParametro.TIPO_NUMERICO, tab2bis1.getT2ECONECO())),
                new DataColumn("ECOTRI.DESCRI", new JdbcParametro(
                    JdbcParametro.TIPO_TESTO, tab2bis1.getT2EDESCRI())),
                new DataColumn("ECOTRI.STIMA", new JdbcParametro(
                    JdbcParametro.TIPO_DECIMALE, tab2bis1.getT2ESTIMA()))});
        ...
        // si richiama l'inserimento dell'occorrenza di tab3 (ECOTRI)
  ...
  protected void manageRettifica(XmlObject documento,
    ...
    // medesima (lavori)
    if (!this.isProgrammaLavori(tab1)) {
      logger.error("La richiesta di rettifica con id ("
          + tab1.getT2IDCONTRI()
          + ") di un programma che in DB e' di forniture/servizi");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
          "Rettifica di un programma che in banca dati e' di forniture/servizi");
      // si termina con questo errore
      return;
    }
   ...
   * @param tab1
   *        elemento tab1 del flusso
   * @return true se il programma nel DB e' di lavori, false altrimenti
   *         (forniture e servizi)
   * @throws SQLException SQLException
   */
  private boolean isProgrammaLavori(final Tab1L1Type tab1) throws SQLException {
  ...
  private void manageIntegrazioneORettifica(
      ...
      SQLException {
    this.deleteDatiFlusso(doc.getRichiestaRichiestaRispostaSincronaApprovazioneProgrammaLavori().getTab1());
    this.insertDatiFlusso(doc, datiAggiornamento);
  }
   ...
   * @throws SQLException SQLException
   */
  private void deleteDatiFlusso(final Tab1L1Type tab1) throws SQLException {
    ...
    this.sqlManager.update("delete from inttri where contri = ?",
        new Object[] { contri });
    this.sqlManager.update("delete from immtrai where contri = ?",
        new Object[] { contri });
    this.sqlManager.update("delete from ecotri where contri = ?",
        new Object[] { contri });
  }
