<%/*
       ...
       * aver prima formalizzato un accordo specifico con EldaSoft.
       ...
       Si usa la form personalizzata per far coincidere login con il codice fiscale
       Quando sarà fatto a livello generale, si potrà togliere questa pagina
...
<gene:template file="scheda-nomenu-template.jsp">
	...
	<c:otherwise>
		<gene:formScheda entita="USRSYS" gestore="it.eldasoft.gene.tags.gestori.submit.GestoreFormRegistrazione" >
			<gene:campoScheda>
				<td colspan="2"><br><b>
				* INFO: Per procedere con la richiesta di registrazione al SITAT compilare la seguente scheda.<br>
				Dopo l'invio della richiesta, inviare fotocopia del proprio documento di identit&agrave; via fax allo 055-438 2281.
				<br><br>
				</b></td>
			</gene:campoScheda>
			<gene:campoScheda>
			...
			<gene:campoScheda campo="LOGIN" campoFittizio="true" definizione="T16;;;;" title="Login" obbligatorio="true" visibile="false" modificabile="${empty requestScope.login}" value="${requestScope.login}" />
	...
	<gene:javaScript>
		...
		document.forms[0].encoding="multipart/form-data";
			...
			$(document).ready(function()
			{
				...
				jQuery.validator.addMethod("controlloCFPIVAente",
				function(value, element) {
					return (checkCodFis(value) || checkCodFisFittizio(value));
				},
				"Il valore specificato non è valido");
			            ...
			            UFFINT_CFEIN:
			           	{
			            	...
			            	controlloCFPIVAente: "Il valore specificato non rispetta il formato previsto",
			...
			function apriModelloAbilitazioneServizio(){
				...
				document.location.href='ModelloRegistrazione.do?' +csrfToken+'&metodo=download';
			}
			...
			function getDescrizioneEnte(valore) {
				...
				var test1 = checkCodFis(cfein);
				var test2 = checkCodFisFittizio(cfein);
				if (cfein != "" && (test1 || test2)) {
					$.ajax({
							type: "GET",
							dataType: "json",
							async: false,
      ...
      $(function() {
				$("#UFFINT_CFEIN").autocomplete({
					...
					source: function( request, response ) {
						changeUFFINT = false;
						var cfamm = $("#UFFINT_CFEIN").val().toUpperCase();
						$.ajax({
							async: false,
							type: "GET",
							dataType: "json",
			...
			function checkCodFisFittizio(cfein) {
				var result = true;
				if (cfein == null || cfein.length == 0)
					return false;
				
				if (cfein.length != 14)
					return false;
					
				var cf = cfein.toUpperCase();
				if (cf.indexOf("CFAVCP-") == 0) {
					var tmp = cf.substring(7);
					validi = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
					for( i = 0; i < tmp.length && result; i++) {
						// Se non e' tra i caratteri validi da errore
						if( validi.indexOf( tmp.charAt(i) ) == -1 ) {
							result = false;
						}
					}
				} else {
