...
<fmt:setBundle basename="AliceResources" />
			...
			<c:forEach items="${listaImmobiliDaTrasfInterv}" var="immobile" varStatus="indiceListaImmobiliDaTrasfInterv">
				<gene:gruppoCampi>
					...
					<c:choose>
						...
						<c:otherwise>
							<gene:campoScheda campo="CUIIMM_${contatore}" entita="IMMTRAI"  campoFittizio="true" definizione="T50" title="Codice univoco immobile" value="${immobile[5]}" modificabile="false" speciale="false" />
						</c:otherwise>
					</c:choose>
					...
					<gene:archivio titolo="Comuni" chiave="" where="" formName="" inseribile="false" 
						...
						schedaPopUp="" campi="TB1.TABCOD3;TABSCHE.TABDESC;TABSCHE.TABCOD3">
						<gene:campoScheda campo="PROVINCIA_${contatore}" campoFittizio="true" definizione="T2;0;Agx15"
							title="Provincia" value="${immobile[8]}" />
						<gene:campoScheda campo="COMUNE_${contatore}" campoFittizio="true" definizione="T120"	title="Comune" value="${immobile[7]}" />
						<gene:campoScheda entita="IMMTRAI" campo="COMIST_${contatore}" campoFittizio="true" definizione="T9;;;;T2COMIST"   title="Codice ISTAT del comune"  defaultValue="${comist}" value="${immobile[6]}" />
					</gene:archivio>

					<gene:campoScheda campo="NUTS_${contatore}" entita="IMMTRAI" campoFittizio="true" definizione="T5" title="Codice NUTS" value="${immobile[12]}" href="#" modificabile="false" speciale="true" >
	...
	<c:forEach var="nuoviImmobili" begin="1" end="5" step="1">
		<gene:gruppoCampi>
			...
			<c:choose>
				...
				<c:otherwise>
					<gene:campoScheda campo="CUIIMM_${contatore}" entita="IMMTRAI"  campoFittizio="true" definizione="T50" title="Codice univoco immobile" modificabile="false" speciale="false" />
				</c:otherwise>
			</c:choose>
			<gene:campoScheda entita="IMMTRAI"
			...
			<gene:campoScheda campo="NUTS_${contatore}" entita="IMMTRAI"  campoFittizio="true" definizione="T5" title="Codice NUTS" href="#" modificabile="false" speciale="true" >
				<gene:popupCampo titolo="Dettaglio codice NUTS" href="#" />
			</gene:campoScheda>
			<gene:campoScheda entita="IMMTRAI"
				campoFittizio="true" 
	...
	$(window).ready(function () {
		...
		_creaLinkAlberoNUTS($("#INTTRI_NUTS").parent(), "${modo}", $("#INTTRI_NUTS"), $("#INTTRI_NUTSview") );
		...
		for (var ii=1; ii <= maxIdImmobileVisualizzabile ; ii++) {
			_creaLinkAlberoNUTS($("#IMMTRAI_NUTS_" + ii).parent(), "${modo}", $("#IMMTRAI_NUTS_" + ii ), $("#IMMTRAI_NUTS_" + ii + "view") );
		}

		$("input[name^='IMMTRAI_NUTS']").attr('readonly','readonly');
		$("input[name^='IMMTRAI_NUTS']").attr('tabindex','-1');
		$("input[name^='IMMTRAI_NUTS']").css('border-width','1px');
		$("input[name^='IMMTRAI_NUTS']").css('background-color','#E0E0E0');
	});
