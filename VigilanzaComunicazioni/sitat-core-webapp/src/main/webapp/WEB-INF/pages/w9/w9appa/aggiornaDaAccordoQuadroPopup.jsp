...
<gene:template file="popup-message-template.jsp">
	<gene:setString name="titoloMaschera"	value="Aggiorna da accordo quadro" />
	<gene:redefineInsert name="corpo">
		<form action="${contextPath}/w9/ConsultaGara.do?metodo=aggiornaDaAccordoQuadro" name="formRiallineaSIMOG" method="post">
			<input type="hidden" name="metodo" value="aggiornaDaAccordoQuadro" />
			<input type="hidden" name="codiceGara" value="${param.codGara}" />
			<input type="hidden" name="codiceLotto" value="${param.codLott}" />
			<input type="hidden" name="numAppa" value="${param.numAppa}" />

			<c:choose>
				<c:when test='${not empty requestScope.RISULTATO and requestScope.RISULTATO eq "OK"}'>
					<br><br>
					Aggiornamento dei dati dell'accordo quadro avvenuto con successo.
					<br><br>
				</c:when>
				<c:when test='${not empty requestScope.RISULTATO and requestScope.RISULTATO eq "KO"}' >
					<br><br>
					Aggiornamento dei dati dell'accordo quadro terminato con errore.
					<br><br>
				</c:when>
				<c:when test='${empty requestScope.RISULTATO}'>
					<br><br>
					Questa funzionalit&agrave; potr&agrave; cambiare i dati dell'accordo quadro rileggendoli dal SIMOG.
