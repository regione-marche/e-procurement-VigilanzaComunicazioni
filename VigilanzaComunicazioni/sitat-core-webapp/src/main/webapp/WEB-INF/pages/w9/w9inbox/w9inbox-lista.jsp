...
<gene:template file="lista-template.jsp" gestisciProtezioni="true" schema="W9" idMaschera="W9INBOX-lista">
	...
	<gene:redefineInsert name="corpo">
		...
		<table class="lista">
			<tr>
				<td>
					<gene:formLista entita="W9INBOX" pagesize="20" tableclass="datilista" gestore="it.eldasoft.w9.tags.gestori.submit.GestoreW9INBOX" sortColumn="-3" gestisciProtezioni="true" >
						...
						<c:if test="${currentRow >= 0}">
							<gene:PopUp variableJs="rigaPopUpMenu${currentRow}" onClick="chiaveRiga='${chiaveRigaJava}'">
								...
								<c:if test='${datiRiga.W9INBOX_STACOM eq "4" and datiRiga.W9FLUSSI_TINVIO2 ne -1}'>
									<c:if test='${gene:checkProt(pageContext, "MASC.VIS.W9.W9INBOX-lista")}'>
										<gene:PopUpItemResource resource="" title="Esegui importazione" href="javascript:eseguiImport('${datiRiga.W9INBOX_IDCOMUN}', '1');" />
									</c:if>
									<c:if	test='${gene:checkProt(pageContext, "MASC.VIS.W9.W9INBOX-lista")}'>
										<gene:PopUpItemResource resource=""	title="Annulla importazione" href="javascript:eseguiImport('${datiRiga.W9INBOX_IDCOMUN}', '2');" />
									</c:if>
								</c:if>
								<c:if test='${datiRiga.W9INBOX_STACOM eq "3" and datiRiga.W9FLUSSI_TINVIO2 ne -1}'>
									<c:if test='${gene:checkProt(pageContext, "MASC.VIS.W9.W9INBOX-lista")}'>
										<gene:PopUpItemResource resource="" title="Riprova importazione" href="javascript:eseguiImport('${datiRiga.W9INBOX_IDCOMUN}', '3');" />
									</c:if>
								</c:if>
								<c:if test='${(datiRiga.W9INBOX_STACOM eq "99" or datiRiga.W9INBOX_STACOM eq "3") and datiRiga.W9FLUSSI_TINVIO2 ne -1}'>
								...
								<c:if test='${datiRiga.W9INBOX_STACOM eq "4" and datiRiga.W9FLUSSI_TINVIO2 eq -1}'>
									<c:if test='${gene:checkProt(pageContext, "MASC.VIS.W9.W9INBOX-lista")}'>
										<gene:PopUpItemResource resource="" title="Accetta richiesta" href="javascript:eseguiImport('${datiRiga.W9INBOX_IDCOMUN}', '4');" />
										<gene:PopUpItemResource resource="" title="Rifiuta richiesta" href="javascript:eseguiImport('${datiRiga.W9INBOX_IDCOMUN}', '2');" />
									</c:if>
								</c:if>
							</gene:PopUp>
						</c:if>
					...
					%>
						...
						<gene:campoLista title="Stato" width="50">
							<center>
								<c:choose>
									...
									<c:when test='${datiRiga.W9INBOX_STACOM eq "99"}'>
						...
						<gene:campoLista campo="NOMEIN" title="Denominazione stazione appaltante" entita="UFFINT" from="W9FLUSSI" where="W9FLUSSI.IDCOMUN = W9INBOX.IDCOMUN AND W9FLUSSI.CFSA = UFFINT.CFEIN"	ordinabile="false" />
						<gene:campoLista campo="DATRIC" title="Data e ora ricezione" headerClass="sortable" />
						<gene:campoLista campo="KEY03" entita="W9FLUSSI" where="W9FLUSSI.IDCOMUN = W9INBOX.IDCOMUN" title="Tipologia flusso" />
						<gene:campoLista campo="CODOGG" entita="W9FLUSSI" where="W9FLUSSI.IDCOMUN = W9INBOX.IDCOMUN" />
						<gene:campoLista campo="TINVIO2" entita="W9FLUSSI" where="W9FLUSSI.IDCOMUN = W9INBOX.IDCOMUN" title="Tipo di invio" />
					</gene:formLista>
				</td>
			</tr>
	...
	$(document).ready(function() {
			document.forms[0].jspPathTo.value="/WEB-INF/pages/w9/w9inbox/w9inbox-lista.jsp";
