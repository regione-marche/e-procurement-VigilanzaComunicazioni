...
<c:set var="isNavigazioneDisattiva" value="${isNavigazioneDisabilitata}" />
	...
	<c:choose>
		...
		<c:when test='${gene:checkProt(pageContext,"FUNZ.VIS.ALT.W9.W9HOME.PIATRI")}'>
			<c:set var="nomeImmagine" value="banner_logo_PT.png"/>
		</c:when>
		<c:otherwise>
			<c:set var="nomeImmagine" value="banner_logo.png"/>
		</c:otherwise>
	...
	<c:if test='${(! empty moduloAttivo) and (! empty profiloUtente) and (isNavigazioneDisattiva ne "1")}' >
		<a href="javascript:goHome('${moduloAttivo}');" title="Torna alla homepage" tabindex="10">
			...
			<img src="${contextPath}/img/${nomeImmagine}" alt="Torna alla homepage" title="Torna alla homepage"><c:if test='${(! empty moduloAttivo) and (! empty profiloUtente) and (isNavigazioneDisattiva ne "1")}' ></a></c:if>
	</div>
