		...
		if(isMethodCall) {
			...
			this.each(function() {
				var instance = instances[$.data(this, "jstree_instance_id")],
					methodValue = (instance && (typeof instance[settings] === "function")) ? instance[settings].apply(instance, args) : instance;
	...
	$(function() {
		...
		var u = navigator.userAgent.toLowerCase(),
			...
			css_string = '' + 
				...
				'.jstree li { display:block; min-height:18px; line-height:18px; white-space:nowrap; margin-left:18px; margin-left:0px; *margin-left:0px; min-width:18px; } ' + 
				'.jstree-rtl li { margin-left:0; margin-right:18px; } ' + 
				'.jstree > ul > li { margin-left:0px; } ' + 
				'.jstree li li { margin-left:18px; *margin-left:18px; } ' +
				'.jstree-rtl > ul > li { margin-right:0px; } ' + 
				'.jstree ins { display:inline-block; text-decoration:none; width:18px; height:18px; height:19px; *height:19px; margin:0 0 0 0; padding:0; } ' + 
		...
		_fn : { 
								...
								o.find("li").each(function () {
									...
									if(th.data("jstree")) {
										$.each(th.data("jstree"), function (plugin, values) {
											if(t.data[plugin] && (typeof t["_" + plugin + "_notify"] === "function")) {
			...
			create_node	: function (obj, position, js, callback, is_loaded) {
				...
				$.each(js.data, function (i, m) {
					tmp = $("<a />");
					if((typeof m === "function")) { m = m.call(this, js); }
	...
	$.jstree.plugin("json_data", {
		...
		_fn : {
			...
			_is_loaded : function (obj) { 
				var s = this._get_settings().json_data;
				obj = this._get_node(obj); 
				return obj == -1 || !obj || (!s.ajax && !s.progressive_render && !(typeof s.data === "function")) || obj.is(".jstree-open, .jstree-leaf") || obj.children("ul").children("li").length > 0;
			},
			refresh : function (obj) {
				obj = this._get_node(obj);
				var s = this._get_settings().json_data;
				if(obj && obj !== -1 && s.progressive_unload && ((typeof s.data === "function") || !!s.ajax)) {
			...
			load_node_json : function (obj, s_call, e_call) {
				...
				switch(!0) {
					...
					case (!s.data && !!s.ajax) || (!!s.data && !!s.ajax && obj && obj !== -1):
						...
						if((typeof s.ajax.url === "function")) { s.ajax.url = s.ajax.url.call(this, obj); }
						if((typeof s.ajax.data === "function")) { s.ajax.data = s.ajax.data.call(this, obj); }
						$.ajax(s.ajax);
						break;
				}
			},
...
(function ($) {
	$.vakata.xslt = function (xml, xsl, callback) {
		...
		if(typeof window.DOMParser !== "undefined" && typeof window.XMLHttpRequest !== "undefined" && typeof window.XSLTProcessor !== "undefined") {
			...
			support = (typeof processor.transformDocument === "function") ? (typeof window.XMLSerializer !== "undefined") : true;
	...
	$.jstree.plugin("xml_data", {
		...
		_fn : {
			...
			_is_loaded : function (obj) { 
				...
				obj = this._get_node(obj);
				return obj == -1 || !obj || (!s.ajax && !(typeof s.data === "function")) || obj.is(".jstree-open, .jstree-leaf") || obj.children("ul").children("li").size() > 0;
			},
		...
		_fn : {
			search : function (str, skip_async) {
				...
				if(!skip_async && s.ajax !== false && this.get_container_ul().find("li.jstree-closed:not(:has(ul)):eq(0)").length > 0) {
					...
					s.ajax.error = error_func;
					s.ajax.success = success_func;
					if((typeof s.ajax.url === "function")) { s.ajax.url = s.ajax.url.call(this, str); }
					if((typeof s.ajax.data === "function")) { s.ajax.data = s.ajax.data.call(this, str); }
...
(function ($) {
	$.vakata.context = {
		...
		exec	: function (i) {
			if((typeof $.vakata.context.func[i] === "function")) {
	...
	$.jstree.plugin("contextmenu", {
		...
		_fn : {
			show_contextmenu : function (obj, x, y) {
				...
				if((typeof i === "function")) { i = i.call(this, obj); }
...
(function ($) {
	$.jstree.plugin("types", {
		__init : function () {
			...
			if(is_ie6 || navigator.userAgent.match("MSIE")) {
				this.get_container()
		...
		_fn : {
			...
			_check : function (rule, obj, opts) {
				...
				if((typeof v === "function")) { v = v.call(this, obj); }
...
(function ($) {
	$.jstree.plugin("html_data", {
		...
		_fn : {
			...
			_is_loaded : function (obj) { 
				obj = this._get_node(obj); 
				return obj == -1 || !obj || (!this._get_settings().html_data.ajax && !(typeof this._get_settings().html_data.data === "function")) || obj.is(".jstree-open, .jstree-leaf") || obj.children("ul").children("li").size() > 0;
			},
		...
		_fn : { 
			_check_unique : function (nms, p, func) {
				...
				if((cnms.length + nms.length) != cnms.concat(nms).sort().join(",,").replace(/(,|^)([^,]+)(,,\2)+(,|$)/g,"$1$2$4").replace(/,,+/g,",").replace(/,$/,"").split(",").length) {
					this._get_settings().uniqueSort.error_callback.call(null, nms, p, func);
					return false;
				}
				return true;
			},
	...
	var nodeInterface = ["getChildren","getChildrenCount","getAttr","getName","getProps"],
		validateInterface = function(obj, inter) {
			...
			$.each(inter, function (i, v) {
				if(!(typeof obj[v] === "function")) { valid = false; return false; }
			});
			return valid;
		};
	$.jstree.plugin("model", {
		...
		_fn : {
			model_done : function (data, callback) {
				...
				$.each(data, function (i, nd) {
					...
					if(_this.data.types && (typeof nd.getType === "function")) {
