...
var myjstreecpvvp = myjstreecpvvp || (function() {
	var _cpvvpargs = {};
	
	return {
		init : function(Args) {
			_cpvvpargs = Args;
			_cpvvpctx = _cpvvpargs[0];
		}
	};
}());
...
CpvVPGrpDiv = [{"grpid": "g1", "grpdesc": "Agricoltura e alimentazione", "grpdiv" : ["03","15","77"]},
     ...
     {"grpid": "g5", "grpdesc": "Edilizia e propriet� immobiliare", "grpdiv" : ["45","70","71","79","90"]},
 ...
 */
function _creaLinkAlberoCpvVP(anchor, modo, cpvvpref, cpvvprefview, cpvdesc) {
	setTimeout(function(){
		
		if (modo == "VISUALIZZA" && cpvvpref.val() != null && cpvvpref.val() != "") {
			cpvvprefview.parent().on("click",function(event) {
				event.preventDefault();
				_popolaFinestraAlberoCpvVP(modo, cpvvpref, cpvvprefview, cpvdesc);
			});
		}
		...
		if (modo != "VISUALIZZA") {
			...
			window[_jsPopUpLinkSet] = creaVocePopUpChiusura(window.location.pathname.substring(0, window.location.pathname.indexOf("/",2))+"/");
			...
			$("body").on("click" ,"#" + _idlinkcpvvp, function(event) {
				event.preventDefault();
				_popolaFinestraAlberoCpvVP(modo, cpvvpref, cpvvprefview, cpvdesc);
			});

			$("body").on("click" ,"#" + _idlinkdelcpvvp, function(event) {
				event.preventDefault();
				cpvvpref.val("");
				cpvvpref.change();
				cpvvprefview.text("");
 ...
 */
function _popolaFinestraAlberoCpvVP(modo, cpvvpref, cpvvprefview, cpvdesc) {
	...
	$('#cpvvpdeletesearch').on("click",function() {
		clearSearchCpvVP();
	});

	$('#cpvvptextsearch').on("keyup",function() {
		delay(function(){
			searchCpvVP();
    	}, 600);
	});
	...
	_caricaAlberoCpvVP(modo, cpvvpref, cpvvprefview, cpvdesc);
 ...
 */
function _associaEventiAlberoCpvVP(modo, cpvvpref, cpvvprefview, cpvdesc) {
	
	// Associa il menu' alla selezione del nodo
    $('#cpvvptree').on('select_node.jstree', function(e,data) {
 ...
 */
function _caricaAlberoCpvVP(modo, cpvvpref, cpvvprefview, cpvdesc) {
    ...
    $("#cpvvptree").jstree(
		{ 
			...
			"themes" : { "theme" : "classic", "url" : _cpvvpctx + "/css/jquery/jstree/themes/classic/style.css"  },
			"ui" : { "select_limit" : 1	},
			"contextmenu" : { 
				"items" : CpvVPMenu,
				"show_at_node" : false
			...
			"json_data" : { 
				"ajax" : {
	                ...
	                url: _cpvvpctx + "/w3/GestioneAlberoCpvVP.do",
	                data : function (n) { 
						return {
    ...
    function CpvVPMenu(node) {
    	if (modo != "VISUALIZZA" && (node.attr("livello") > 3 || node.attr("tiponodo") == "CPV" )) {
	        var items = {
	        	selItem: {
	           		label: "Seleziona CPV e chiudi",
		           		action: function (obj) { _selcpvvp(node, cpvvpref, cpvvprefview, cpvdesc); },
		           		_disabled : false
		            }
			    };
	        }
	 ...
	 */
	 _associaEventiAlberoCpvVP(modo, cpvvpref, cpvvprefview, cpvdesc);
 ...
 * Selezione del CPV
 */
function _selcpvvp(node, cpvvpref, cpvvprefview, cpvdesc) {
	var cpvvpcod = node.attr("cpvvpcod");
	var cpvvpdesc = node.attr("cpvvpdesc")
	cpvvpref.val(cpvvpcod);
	cpvvpref.change();
	cpvvprefview.text(cpvvpcod);
	cpvdesc.val(cpvvpdesc);
	cpvdesc.change();
