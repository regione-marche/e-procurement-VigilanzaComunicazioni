...
function _creaLinkAlberoNUTS(anchor, modo, nutsref, nutsrefview) {
	setTimeout(function(){
		...
		if (modo == "VISUALIZZA" && nutsref.val() != null && nutsref.val() != "") {
			nutsrefview.parent().on("click",function(event) {
		...
		if (modo != "VISUALIZZA") {
			...
			$("body").on("click" ,"#" + _idlinknuts, function(event) {
				event.preventDefault();
				_popolaFinestraAlberoNUTS(modo, nutsref, nutsrefview);
			});

			$("body").on("click" ,"#" + _idlinkdelnuts, function(event) {
				event.preventDefault();
				nutsref.val("");
				nutsref.change();
				nutsrefview.text("");
...
function _popolaFinestraAlberoNUTS(modo, nutsref, nutsrefview) {
	...
	$('#nutsdeletesearch').on("click",function() {
		clearSearchNUTS();
	});

	$('#nutstextsearch').on("keyup",function() {
		delay(function(){
			searchNUTS();
    	}, 600);
	});
...
function _associaEventiAlberoNUTS(modo, nutsref, nutsrefview) {
    ...
    $('#nutstree').on('select_node.jstree', function(e,data) {
