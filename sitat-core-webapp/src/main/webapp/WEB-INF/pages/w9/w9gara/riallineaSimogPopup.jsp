...
<gene:template file="popup-message-template.jsp">
	...
	<gene:redefineInsert name="corpo">
		<form action="${contextPath}/w9/ConsultaGara.do?metodo=riallineaSchedaGara" name="formRiallineaSIMOG" method="post">
			<c:choose>
				<c:when test='${not empty codGara}'>
					<input type="hidden" name="idAvGara" value="${idAvGara}" />
					<input type="hidden" name="codGara" value="${codGara}" />
				</c:when>
				<c:otherwise>
					<input type="hidden" name="idAvGara" value="${param.idAvGara}" />
					<input type="hidden" name="codGara" value="${param.codGara}" />
				</c:otherwise>
			</c:choose>
			
			<c:choose>
				<c:when test='${not empty codiceCIG}'>
					<input type="hidden" name="codiceCIG" value="${codiceCIG}" />	
				</c:when>
				<c:otherwise>
...
<c:choose>
		<c:when test='${empty sessionScope.riallineaSchedaGaraBean}'>
			<c:choose>
				<c:when test='${not empty requestScope.RISULTATO and requestScope.RISULTATO eq "OK"}'>
					<br><br>
					Aggiornamento dei dati della gara e dei lotti avvenuto con successo.
					<br><br>
				</c:when>
				<c:when test='${not empty requestScope.RISULTATO and requestScope.RISULTATO eq "KO"}' >
					<br><br>
					Aggiornamento dei dati della gara e dei lotti terminato con errore.
					<br><br>
				</c:when>
				<c:when test='${empty requestScope.RISULTATO}'>
					<br><br>
					Questa funzionalit&agrave; potr&agrave; cambiare i dati della gara e dei lotti rileggendoli dal SIMOG.
		...
		<c:otherwise>
			<br><br>
				...
				Aggiornamento dei dati del CIG ${codiceCIG} in corso...
			<br><br>
		</c:otherwise>
	...
	<c:if test='${not empty codiceCIG and codiceCIG ne "" and requestScope.RISULTATO eq "OK"}'>
	$(document).ready(function () {
				bloccaRichiesteServer();
				setTimeout("document.formRiallineaSIMOG.submit();", 350);
	});
	
	$(function() {
		  $("#progressbar").progressbar({
		    value: ${100 *(sessionScope.riallineaSchedaGaraBean.numeroLottiImportati) / sessionScope.riallineaSchedaGaraBean.numeroTotaleLotti} 
		  }); //.delay( 250 );
	});
	</c:if>
	</gene:javaScript>
