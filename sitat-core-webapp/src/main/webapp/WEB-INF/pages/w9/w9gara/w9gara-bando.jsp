...
<gene:formScheda entita="W9GARA" gestisciProtezioni="true" plugin="it.eldasoft.w9.tags.gestori.plugin.GestoreW9GARA" gestore="it.eldasoft.w9.tags.gestori.submit.GestoreW9GARA">
	...
	<c:choose>	
		<c:when test="${modoAperturaScheda eq 'VISUALIZZA' and not(isDocBandoEstratti)}">
			<gene:campoScheda>
		...
		<c:when test="${modoAperturaScheda eq 'VISUALIZZA' and isDocBandoEstratti}">
			...
			<jsp:include page="/WEB-INF/pages/commons/interno-scheda-multipla.jsp" >
				...
				<jsp:param name="arrayCampi" value="'W9DOCGARA_CODGARA_', 'W9DOCGARA_NUMDOC_', 'W9DOCGARA_NUM_PUBB_', 'W9DOCGARA_TITOLO_', 'W9DOCGARA_URL_', 'NOME_FILE_', 'VISUALIZZA_FILE_', 'selezioneFile_'"/> 		
...
<c:if test="${modo eq 'MODIFICA'}">
			...
			if (isObjShow("rowW9DOCGARA_TITOLO_" + er)) {
				if (getValue("W9DOCGARA_TITOLO_" + er) != "") {
					if(document.getElementById("selFile[" + er + "]") == null) {
						if (isObjShow("rowW9DOCGARA_URL_" + er) && getValue("W9DOCGARA_URL_" + er) == "") {
							alert("Valorizzare il campo 'URL della documentazione");
							continua = false;
						}
					} else if(document.getElementById("selFile[" + er + "]") != null && getValue("selFile[" + er + "]") == "") {
	...
	function visualizzaFileAllegato(numeroDoc, numeroPubb) {
		var href = "${pageContext.request.contextPath}/w9/VisualizzaAllegato.do";
		document.location.href = href+"?" + csrfToken + "&codgara=${codiceGara}&numdoc=" + numeroDoc + "&numPubb=" + numeroPubb + "&tab=gara";
	}
