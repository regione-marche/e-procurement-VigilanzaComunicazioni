...
<gene:template file="popup-template.jsp">
	...
	<gene:javaScript>
		...
		function reloadCPVNUTS() {
			...
			document.location.href="${pageContext.request.contextPath}/ApriPagina.do?" + csrfToken + "&href=w3/commons/dettaglio-codice-nuts.jsp&modo=MODIFICA&campo=${param.campo}&valore="+getValue("CODICE");
		}
		
		function conferma() {
			var codice = getValue("CODICE");
			var codiceRicalcolato = ricalcolaNUTS();
			if (codice == codiceRicalcolato) {
				opener.setValue("${param.campo}",getValue("CODICE"));
				window.close();
			} else {
				outMsg("Codice NUTS errato", "ERR");
			}
		}
