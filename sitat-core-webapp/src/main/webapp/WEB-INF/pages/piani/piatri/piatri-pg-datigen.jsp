...
<c:set var="nonPubblicato" value="true"/>
<c:if test='${modo ne "NUOVO"}'>
	<c:set var="nonPubblicato" value='${gene:callFunction2("it.eldasoft.sil.pt.tags.funzioni.IsNotPubblicatoFunction", pageContext, key)}'/>
</c:if>
...
<gene:formScheda entita="PIATRI" gestisciProtezioni="true" gestore="it.eldasoft.sil.pt.tags.gestori.submit.GestorePIATRI">
	<gene:gruppoCampi idProtezioni="GEN">
		...
		<gene:campoScheda campo="NORMA" visibile="false" defaultValue="1" />
		...
		<gene:campoScheda campo="TIPROG" defaultValue='${gene:if((not empty tiprog ),tiprog,3)}'
					modificabile="${modoAperturaScheda ne 'MODIFICA'}" title="Tipo programma" obbligatorio="true"/>
		...
		<gene:campoScheda campo="PUBBLICA" defaultValue="1" modificabile="${nonPubblicato}"/>
		...
		<gene:archivio titolo="TECNICI"
			...
			<gene:campoScheda campo="RESPRO" title="Codice del responsabile del programma" visibile="false"/>
			<gene:campoScheda campo="NOMTEC" entita="TECNI" where="TECNI.CODTEC=PIATRI.RESPRO"
				title="Nome del responsabile del programma" definizione="T61" />
		</gene:archivio>
