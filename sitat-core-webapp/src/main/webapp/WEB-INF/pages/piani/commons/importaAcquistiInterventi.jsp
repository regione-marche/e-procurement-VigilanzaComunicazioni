...
<gene:setString name="titoloMaschera" value='Importazione Interventi/Aquisti' />
...
<form method="post" enctype="multipart/form-data" name="formImportfile" action="${pageContext.request.contextPath}/piani/ImportaAcquistiInterventi.do">
	<c:choose>
	...
	<table class="dettaglio-notab">
	    <tr>
	    	<td colspan="2">
	    	  <p>
		    	  <br>Questa funzione permette di caricare interventi e acquisti nel programma con chiave indicata.
				  <br>&nbsp;
				  <br>&nbsp;
			  </p>
			</td>
		...
		<tr>
			<td class="etichetta-dato">Contri (*)</td>
			<td class="valore-dato"><input type="text" name="contri" id="contri" /></td>
		</tr>
		<tr>
		...
		if(document.formImportfile.selezioneFile) {
			...
			if(nomeCompletoFile != ""){
				if(nomeCompletoFile.substr(nomeCompletoFile.lastIndexOf('.')+1).toUpperCase() == "XLS" || nomeCompletoFile.substr(nomeCompletoFile.lastIndexOf('.')+1).toUpperCase() == "XLSX") {
					...
					alert("Il file selezionato non &egrave; nel formato valido. Deve avere estensione XLS o XLSX");
