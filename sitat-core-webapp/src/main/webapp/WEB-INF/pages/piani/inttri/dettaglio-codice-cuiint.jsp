...
<c:set var="tipoin" value="${param.tipoin}" scope="request" />
...
<c:choose>
	<c:when test='${empty param.cui}'>
		...
		<c:set var="cui" value="${cfamm}${anno}${progr}" />
	</c:when>
	<c:otherwise>
				...
				$.ajax({
					...
					success: function( data ) {
						if (data[0] == null & data[1] == null) {
							opener.setValue("INTTRI_CUIINT", getValue("CUIINT"));
							window.close();
						} else {
							if (data[0] != null) {
								outMsg(data[0], "ERR");
