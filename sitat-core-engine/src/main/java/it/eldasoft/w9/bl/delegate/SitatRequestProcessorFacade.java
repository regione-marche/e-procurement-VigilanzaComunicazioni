...
public class SitatRequestProcessorFacade {
  ...
  private void insertW9FLUSSI(final String xmlEvento, final FaseType faseInvio,
    ...
    if (faseInvio.getW3FASEESEC() == W3020Type.X_988 || faseInvio.getW3FASEESEC() == W3020Type.X_13) {
      ...
      } else if (faseInvio.getW3FASEESEC() == W3020Type.X_982) {
        try {
          codogg = (RichiestaRichiestaRispostaSincronaApprovazioneProgrammaTriennaleOperePubblicheDocument.Factory.parse(
              xmlEvento)).getRichiestaRichiestaRispostaSincronaApprovazioneProgrammaTriennaleOperePubbliche().getProgramma().getT2IDCONTRI();
        } catch (XmlException e) {
          logger.error("Errore nel parsing del XML del messaggio (" + W3020Type.X_982 + ")", e);
        }
      } else if (faseInvio.getW3FASEESEC() == W3020Type.X_981) {
        try {
          codogg = (RichiestaRichiestaRispostaSincronaApprovazioneProgrammaBiennaleAcquistiDocument.Factory.parse(
              xmlEvento)).getRichiestaRichiestaRispostaSincronaApprovazioneProgrammaBiennaleAcquisti().getProgramma().getT2IDCONTRI();
        } catch (XmlException e) {
          logger.error("Errore nel parsing del XML del messaggio (" + W3020Type.X_981 + ")", e);
        }
      } else {
        try {
