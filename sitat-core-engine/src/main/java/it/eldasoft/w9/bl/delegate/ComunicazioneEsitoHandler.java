...
public class ComunicazioneEsitoHandler extends AbstractRequestHandler {
  ...
  protected boolean isTest(final XmlObject documento) {
    RichiestaRichiestaRispostaSincronaComunicazioneEsitoDocument doc =
      (RichiestaRichiestaRispostaSincronaComunicazioneEsitoDocument) documento;
  ...
  protected void managePrimoInvio(final XmlObject documento,
      final DataColumnContainer datiAggiornamento, final boolean ignoreWarnings)
    ...
    if (!this.existsLotto(fase)) {
      ...
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
          "Primo invio di una comunicazione esito per un lotto non esistente in banca dati");
      return;
    }
    
    if (this.existsFase(fase, CostantiW9.COMUNICAZIONE_ESITO)) {
      logger.error("La richiesta di primo invio comunicazione esito gia' presente nel DB (cig = "
          + fase.getW3FACIG() + ")");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
          "Primo invio di una comunicazione esito gia' presente in banca dati");
      // si termina con questo errore
      return;
    }
    
    if (! ignoreWarnings) {
    	if (! this.isFaseVisualizzabile(fase, CostantiW9.COMUNICAZIONE_ESITO)) {
    		logger.error("La richiesta di primo invio di comunicazione esito non e' prevista per il CIG=" + fase.getW3FACIG());
        this.setEsito(datiAggiornamento, StatoComunicazione.STATO_WARNING.getStato(),
          "Primo invio di una comunicazione esito non previsto");
        // si termina con questo errore
        return;
    	}
    }
  ...
  @Override
  protected void manageRettifica(final XmlObject documento,
      final DataColumnContainer datiAggiornamento, final boolean ignoreWarnings)
      throws GestoreException, SQLException {
    ...
    if (!this.existsLotto(fase)) {
      logger.error("La richiesta di rettifica  di un lotto non presente in DB (cig = "
          + fase.getW3FACIG() + ")");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
          "Rettifica di una Comunicazione esito per un lotto non esistente in banca dati");
      return;
    }
    
    if (!this.existsFase(fase, CostantiW9.COMUNICAZIONE_ESITO)) {
      logger.error("La richiesta di rettifica della comunicazione esito non presente in DB (cig = "
      		+ fase.getW3FACIG() + ")");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
          "Rettifica di una Comunicazione esito non presente in banca dati");
      // si termina con questo errore
      return;
    }
  ...
  private void insertDatiFlusso(final RichiestaRichiestaRispostaSincronaComunicazioneEsitoDocument doc,
    ...
    // Se l'esito del lotto e' aggiudicato si invoca il metodo AbstractRequestHandler.updateW9Fasi,
    // altrimenti si invoca la sovrascrittura della stesso metodo ComunicazioneEsitoHandler.updateW9Fasi
    // per settare il campo W9FASI.DAEXPORT='1' ed effettuare poi l'invio di questa informazione al SIMOG 
    ...
    DataColumn codiceGara = new DataColumn("W9ESITO.CODGARA", new JdbcParametro(
        JdbcParametro.TIPO_NUMERICO, codGara));
    DataColumn numeroLotto = new DataColumn("W9ESITO.CODLOTT", new JdbcParametro(
        JdbcParametro.TIPO_NUMERICO, codLott));
   
    DataColumnContainer dccLotto = new DataColumnContainer(
        new DataColumn[] {codiceGara, numeroLotto});
    
    dccLotto.addColumn("W9ESITO.ESITO_PROCEDURA", JdbcParametro.TIPO_NUMERICO,
        new Long(tab30.getW9LOESIPROC().toString()));
    if (tab30.isSetW3ESDVERB()) {
    	dccLotto.addColumn("W9ESITO.DATA_VERB_AGGIUDICAZIONE", JdbcParametro.TIPO_DATA,
        tab30.getW3ESDVERB().getTime());
    }
    if (tab30.isSetW3ESIMPAGGI()) {
      dccLotto.addColumn("W9ESITO.IMPORTO_AGGIUDICAZIONE", JdbcParametro.TIPO_DECIMALE,
          tab30.getW3ESIMPAGGI());
    }
    if (tab30.isSetFile()) {
    ...
    if (tab192 != null) {
      Long numeroRecordW9PUES = (Long) this.sqlManager.getObject(
    ...
    // 27/02/2012: Al contrario degli altri flussi, l'import del flusso Comunicazione esito
    // non deve aggiornare il flag W9LOTT.DAEXPORT ad uno per l'esportazione dati SIMOG
  ...
  private void manageIntegrazioneORettifica(
      final RichiestaRichiestaRispostaSincronaComunicazioneEsitoDocument doc,
      final DataColumnContainer datiAggiornamento) throws SQLException,
      GestoreException {
    this.deleteDatiFlusso(doc.getRichiestaRichiestaRispostaSincronaComunicazioneEsito().getFase());
  ...
  private void deleteDatiFlusso(final FaseEstesaType fase) throws SQLException, GestoreException {
    ...
    this.sqlManager.update("delete from w9esito where codgara = ? and codlott = ?",
        new Object[] { codGara, codLott }, 1);
    this.sqlManager.update("delete from w9pues where codgara = ? and codlott = ? and num_iniz = 1",
        new Object[] { codGara, codLott });
   ...
   * 
   * Metodo di AbstractRequestHandler sovrascritto per personalizzare l'inserimento/aggiornamento
   * del record in W9FASI per la fase Comunicazione Esito: infatti solo se l'esito del lotto
   * e' non aggiudicato, bisogna valorizzare il campo W9FASI.DAEXPORT='1' in modo da inviare
   * al SIMOG questa informazione nella valorizzazione dei DatiComuni.
   * 
   * @param codGara
   *        codice gara
   * @param codLott
