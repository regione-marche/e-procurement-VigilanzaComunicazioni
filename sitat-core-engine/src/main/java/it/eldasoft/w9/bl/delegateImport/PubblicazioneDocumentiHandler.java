...
public class PubblicazioneDocumentiHandler extends AbstractRequestHandler {
	...
	protected FaseType getFaseInvio(XmlObject documento) {
		RichiestaRichiestaRispostaSincronaPubblicazioneDocumentiDocument doc = (RichiestaRichiestaRispostaSincronaPubblicazioneDocumentiDocument) documento;
		return doc.getRichiestaRichiestaRispostaSincronaPubblicazioneDocumenti().getFase();
	}
	...
	protected void managePrimoInvio(XmlObject documento, DataColumnContainer datiAggiornamento, boolean ignoreWarnings) throws GestoreException, SQLException {
		...
		if (UtilitySITAT.existsGara(idavGara, this.sqlManager)) {
			if (UtilitySITAT.existsPubblicazione(idavGara, numeroPubblicazione, this.sqlManager)) {
				logger.error("La richiesta di primo invio ha gia' una pubblicazione documenti" + " con CODGARA=" + UtilitySITAT.getCodGaraByIdAvGara(idavGara, sqlManager) + ", NUM_PUBB="
						+ numeroPubblicazione + " presente in DB");
				this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(), "Primo invio di pubblicazione documenti gia' presente in banca dati");
				return;
			} else {
				// TODO codice da abilitare e da testare per la 3.0.0 fase 2 o
				// versioni successive
				// Commentato in fase 1
				// Questo codice controlla che tutti i CIG presenti nel XML
				// ricevuto
				// siano lotti della gara sulla base dati di ORT
				/*
				 * String[] arrayCIG =
				 * doc.getRichiestaRichiestaRispostaSincronaPubblicazioneDocumenti
				 * ().getCigLottiArray(); int numeroCIG = arrayCIG.length;
				 * 
				 * String sqlParamCIG = null; StringBuffer strBufferCig = null;
				 * if (numeroCIG > 1) { strBufferCig = new StringBuffer("'");
				 * for (String strCig : arrayCIG) { strBufferCig.append(strCig);
				 * strBufferCig.append("','"); } sqlParamCIG =
				 * strBufferCig.substring(0, strBufferCig.length()-2); } else {
				 ...
				 * "l.CODGARA=g.CODGARA and g.IDAVGARA=? and l.CIG in (" +
				 * sqlParamCIG + ")", new Object[] { idavGara });
				 * 
				 * if (numeroCIG != numeroLottiDiGara) { logger.error(
				 * "La richiesta di primo invio ha dei CIG che non appartengono alla gara con IDAVGARA="
				 * + idavGara + ". In particolare i CIG " + sqlParamCIG +
				 * " non appartengono tutti alla gara con CODGARA=" +
			...
			logger.error("La richiesta di primo invio di una pubblicazione documenti" + " relativi ad una gara non presente in base dati (Gara con IDAVGARA=" + idavGara + " inesistente in DB");
			this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(), "Primo invio di pubblicazione documenti di una gara non presente in banca dati");
			return;
		}
	 ...
	 */
	private void insertDatiFlusso(RichiestaRichiestaRispostaSincronaPubblicazioneDocumentiDocument doc, DataColumnContainer datiAggiornamento) throws SQLException, GestoreException {

		RichiestaRichiestaRispostaSincronaPubblicazioneDocumentiType richiesta = doc.getRichiestaRichiestaRispostaSincronaPubblicazioneDocumenti();

		PubblicazioneType pubblicazione = richiesta.getPubblicazione();
		if (pubblicazione!= null && pubblicazione.getW3IDGARA() != null) {
			String idavGara = pubblicazione.getW3IDGARA();
			Long numeroPubblicazione = new Long(pubblicazione.getW9PBNUMPUBB());
			...
			datiAggiornamento.addColumn("W9PUBBLICAZIONI.TIPDOC", JdbcParametro.TIPO_NUMERICO, new Long(pubblicazione.getW9PBTIPDOC()));
			...
			if (pubblicazione.isSetW9PBDATAST()) {
				datiAggiornamento.addColumn("W9PUBBLICAZIONI.DATA_STIPULA", JdbcParametro.TIPO_TESTO, pubblicazione.getW9PBDATAST().getTime());
			}
			...
			for (int i = 0; i < arrayCIG.length; i++) {
				Long codlott = (Long) sqlManager.getObject("select CODLOTT from W9LOTT where CODGARA=? and CIG=?", new Object[] { codiceGara, arrayCIG[i] });
				if (codlott != null) {
					datiAggiornamento.setValue("W9PUBLOTTO.CODLOTT", codlott);
					datiAggiornamento.insert("W9PUBLOTTO", this.sqlManager);
				} else {
					logger.error("La pubblicazione CODGARA=" + codiceGara + ", NUM_PUBB=" + numeroPubblicazione + " non e' associabile al CIG=" + arrayCIG[i] + " perche' relativo ad un'altra gara.");
				}
			}
