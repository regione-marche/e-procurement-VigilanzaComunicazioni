 ...
 */
public class ConclusioneContrattoAppaltoSopra150000Handler extends
AbstractRequestHandler {
  ...
  throws GestoreException, SQLException {
    ...
    if (!this.existsLotto(fase)) {
      ...
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      		"Primo invio di conclusione appalto di un lotto non presente in banca dati");
      continua = false;
    }
    ...
    if (continua && !this.existsAppalto(fase)) {
      logger.error("La richiesta di primo invio di un lotto non aggiudicato in DB (CIG="
            + fase.getW3FACIG() + ")");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      		"Primo invio di conclusione contratto di un lotto non aggiudicato in banca dati");
      continua = false;
    }

    if (continua && !this.isFaseAbilitata(fase, CostantiW9.CONCLUSIONE_CONTRATTO_SOPRA_SOGLIA)) {
    	logger.error("La richiesta di primo invio della conclusione contratto non abilitata (cig = " + fase.getW3FACIG() + ")");
    	this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(), 
    			"Primo invio conclusione contratto non abilitato");
    	continua = false;
    }


    if (continua && this.existsConclusione(fase)) {
      logger.error("La richiesta di primo invio di un lotto avente gia' una fase conclusione sul DB (CIG="
          + fase.getW3FACIG() + ")");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      		"Primo invio di conclusione appalto di un lotto avente gia' la fase conclusione in banca dati");
      continua = false;
    }
    
    if (continua && !this.existsInizio(fase)) {
      Tab21Type tab21 = doc.getRichiestaRichiestaRispostaSincronaConclusioneContrattoSopra150000().getTab21();
      if ("2".equals(this.getFlagSNDB(tab21.getW3CSINTANTIC()).toString())) {
        logger.error("La richiesta di primo invio di un lotto (CIG="
            + fase.getW3FACIG()
            + ") senza fase una inziale sul DB e la conclusione non e' una interruzione anticipata");      
        this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
        		"Primo invio di conclusione appalto, senza interruzione anticipata, di un lotto senza una fase inziale in banca dati");
        continua = false;
      }
    }
    ...
    if (continua && !ignoreWarnings) {
    	if (! this.isFaseVisualizzabile(fase, CostantiW9.CONCLUSIONE_CONTRATTO_SOPRA_SOGLIA)) {
        ...
        this.setEsito(datiAggiornamento, StatoComunicazione.STATO_WARNING.getStato(),
          "Primo invio di conclusione del contratto non previsto");
        continua = false;
      }
    }
  ...
  throws GestoreException, SQLException {
    ...
    if (!this.existsLotto(fase)) {
      logger.error("La richiesta di rettifica di un lotto non presente in DB (CIG"
          + fase.getW3FACIG() + ")");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      		"Rettifica di conclusione appalto di un lotto non presente in banca dati");
      continua = false;
    }
    ...
    if (continua && !this.existsAppalto(fase)) {
      logger.error("La richiesta di rettifica di un lotto non aggiudicato in DB (cig = "
          + fase.getW3FACIG() + ")");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      		"Rettifica di conclusione appalto di un lotto non aggiudicato in banca dati");
      continua = false;
    }

    if (continua && !this.existsConclusione(fase)) {
      logger.error("La richiesta di rettifica fase conclusione semplificata di un lotto avente gia' una fase conclusione sul DB (CIG="
          + fase.getW3FACIG() + ")");
      this.setEsito(datiAggiornamento, StatoComunicazione.STATO_ERRATA.getStato(),
      		"Rettifica di conclusione appalto di un lotto avente gia' una fase conclusione in banca dati");
      continua = false;
    }
  ...
  private void manageIntegrazioneORettifica(
      RichiestaRichiestaRispostaSincronaConclusioneContrattoSopra150000Document doc,
      DataColumnContainer datiAggiornamento) throws SQLException,
      GestoreException {

    this.deleteDatiFlusso(doc.getRichiestaRichiestaRispostaSincronaConclusioneContrattoSopra150000().getFase());
  ...
  throws SQLException, GestoreException {
    ...
    if (primoInvio) {
	    ...
	    if (tab21.isSetW9FASCHLOC()) {
	    	idSchedaLocale = tab21.getW9FASCHLOC();
	  	}
	  	if (tab21.isSetW9FASCHSIM()) {
	  		idSchedaSimog = tab21.getW9FASCHSIM();
	  	}
    ...
    dccConc.addColumn("W9CONC.INTANTIC", JdbcParametro.TIPO_TESTO,
        this.getFlagSNDB(tab21.getW3CSINTANTIC()).toString());

    if (tab21.isSetW3FLAGPOL()) {
      dccConc.addColumn("W9CONC.FLAG_POLIZZA", JdbcParametro.TIPO_TESTO,
          this.getFlagSNDB(tab21.getW3FLAGPOL()).toString());
    }
    if (tab21.isSetW9CODVERCON()) {
      dccConc.addColumn("W9CONC.DATA_VERBALE_CONSEGNA", JdbcParametro.TIPO_DATA,
          tab21.getW9CODVERCON().getTime());
    }
    if (tab21.isSetW9COTERCONT()) {
      dccConc.addColumn("W9CONC.TERMINE_CONTRATTO_ULT", JdbcParametro.TIPO_DATA,
          tab21.getW9COTERCONT().getTime());
    }
    if (tab21.isSetW9COGGPROR()) {
      dccConc.addColumn("W9CONC.NUM_GIORNI_PROROGA", JdbcParametro.TIPO_NUMERICO,
          new Long(tab21.getW9COGGPROR()));
    }
    
    if (tab21.isSetW9COORELAVO()) {
    	dccConc.addColumn("W9CONC.ORE_LAVORATE", JdbcParametro.TIPO_DECIMALE,
    	          new Double(tab21.getW9COORELAVO()));
    }
    if (tab21.isSetW3DATAULT()) {
      dccConc.addColumn("W9CONC.DATA_ULTIMAZIONE", JdbcParametro.TIPO_DATA,
          tab21.getW3DATAULT().getTime());
    }
    if (tab21.isSetW3DATARIS()) {
      dccConc.addColumn("W9CONC.DATA_RISOLUZIONE", JdbcParametro.TIPO_DATA,
          tab21.getW3DATARIS().getTime());
    }
    if (tab21.isSetW3ONERIRI()) {
      dccConc.addColumn("W9CONC.ONERI_RISOLUZIONE",
          JdbcParametro.TIPO_DECIMALE, tab21.getW3ONERIRI());
    }
    if (tab21.isSetW3FLAGONE()) {
      dccConc.addColumn("W9CONC.FLAG_ONERI", JdbcParametro.TIPO_TESTO,
          tab21.getW3FLAGONE().toString());
    }
    if (tab21.isSetW3IDMOTI1()) {
      dccConc.addColumn("W9CONC.ID_MOTIVO_INTERR",
          JdbcParametro.TIPO_NUMERICO, new Long(tab21.getW3IDMOTI1()));
    }
    if (tab21.isSetW3IDMOTI2()) {
      dccConc.addColumn("W9CONC.ID_MOTIVO_RISOL", JdbcParametro.TIPO_NUMERICO,
          new Long(tab21.getW3IDMOTI2()));
    }
    if (tab21.isSetW3NUMINFO()) {
      dccConc.addColumn("W9CONC.NUM_INFORTUNI", JdbcParametro.TIPO_NUMERICO,
        new Long(tab21.getW3NUMINFO()));
    }
    if (tab21.isSetW3NUMMORT()) {
      dccConc.addColumn("W9CONC.NUM_INF_MORT", JdbcParametro.TIPO_NUMERICO,
        new Long(tab21.getW3NUMMORT()));
    }
    if (tab21.isSetW3NUMPERM()) {
      dccConc.addColumn("W9CONC.NUM_INF_PERM", JdbcParametro.TIPO_NUMERICO,
        new Long(tab21.getW3NUMPERM()));
    }
  ...
  /**
   * Verifica se esiste in banca dati una fase iniziale per il lotto indicato a
  ...
  private boolean existsInizio(FaseEstesaType fase) throws SQLException {
    Long numeroOccorrenze = (Long) this.sqlManager.getObject(
        ...
        + "and w9iniz.num_appa = ? and w9lott.cig = ?", new Object[] { new Long(fase.getW9NUMAPPA()), fase.getW3FACIG() });
  ...
  private boolean existsConclusione(FaseEstesaType fase) throws SQLException {
    Long numeroOccorrenze = (Long) this.sqlManager.getObject(
        ...
        + "and w9conc.num_appa = ? and w9lott.cig = ?", new Object[] { new Long(fase.getW9NUMAPPA()), fase.getW3FACIG() });
    return numeroOccorrenze.intValue() > 0;
  }
