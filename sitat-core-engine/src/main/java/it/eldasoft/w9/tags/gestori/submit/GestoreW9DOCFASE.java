...
/**
 * Gestore per salvataggio occorrenze in W9DOCFASE
 * 
 */
public class GestoreW9DOCFASE extends AbstractGestoreChiaveNumerica {
  ...
  public String[] getAltriCampiChiave() {
    return new String[]{"CODGARA", "CODLOTT", "FASE_ESECUZIONE", "NUM_FASE"};
  }
            ...
            if (updateOccorrenza) {
              ...
              if (formFile != null) {
                if (formFile.getFileSize() > 0 && formFile.getFileSize() <= MAX_FILE_SIZE) {
                  ...
                  newDataColumnContainer.addColumn("W9DOCFASE.FILE_ALLEGATO",
                  ...
                  if (formFile.getFileSize() == 0 && newDataColumnContainer.getLong(gestore.getCampoNumericoChiave()) == null) {
                    ...
                    throw new GestoreException("Errore nell'inserimento del file \""
                        + formFile.getFileName()
                        + "\": non e' possibile inserire documenti di dimensione maggiore di 10 Mb", "upload.overflow", null);
                  }
                }
              }
