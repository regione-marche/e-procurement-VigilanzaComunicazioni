...
public class GestoreCampoW9GaraTipoApp extends AbstractGestoreCampo {
  ...
  protected void initGestore() {
    ...
    if (codiceGara != null && !codiceGara.equals("")) {
      ...
      try {
        ...
        if (versioneSimog == 1) {
          listaTabellati = (List<Tabellato>) sqlManager.getElencoTabellati(
            "select TAB1TIP, TAB1DESC from TAB1 where TAB1COD='W3999' and TAB1TIP <= 12 order by TAB1TIP asc", null);
        } else {
          listaTabellati = (List<Tabellato>) sqlManager.getElencoTabellati(
            "select TAB1TIP, TAB1DESC from TAB1 where TAB1COD='W3999' and TAB1ARC is null order by TAB1TIP asc", null);
        }
